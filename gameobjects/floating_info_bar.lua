...
local draw_menu = function(ai_client, p_open)
    if ig.Begin("Demo: Floating Info Bar", p_open) then
        ...
        ig.NewLine()
        local vec = ai_client:FloatingInfoBars()
        ig.SliderInt("##Spellslot", v_floating_info_bar, 1, #vec, "floating_info_bars[%d]", ig.lib.ImGuiSliderFlags_NoInput)
        if v_floating_info_bar[0] > #vec then
            v_floating_info_bar[0] = 1
        end
        ig.NewLine()
        if #vec > 0 then
            local info = vec[v_floating_info_bar[0]]
            ...
            ig.Text("\t\t\t") ig.SameLine()
            ig.SliderInt("##UIElement", v_ui_element, 1, #vec, "vec[%d]", ig.lib.ImGuiSliderFlags_NoInput)
            if v_ui_element[0] > #vec then v_ui_element[0] = 1 end
            local el = vec[v_ui_element[0]]
            format_value("\t\t\t["..v_ui_element[0].."]", el)
