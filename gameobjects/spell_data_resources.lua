...
local draw_menu = function(aiclient, p_open)
    if ig.Begin("Demo: SpellDataResource", p_open) then
        ...
        if show_basic_attacks[0] then
            ...
            ig.SliderInt("##SpellDataResourceBasicAttacks", v_basic_attack, 1, #basic_attacks, "basic_attack[%d])", ig.lib.ImGuiSliderFlags_NoInput)
            local basic_attack = basic_attacks[v_basic_attack[0]]
